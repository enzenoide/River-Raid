cenario:      
	lui  $16, 0x1001
    	...
    	jal DesenhaHUD
    	jal DesenhaMapa
    	lui $3,0x1001  # Endere�o do aviao
    	addi $3,$3,45312
    	jal DesenhaAviao 
...
Entidades:
	...
	lui $24,0x1001
    	...
    	lui $25,0x1001
    	addi $25,$25,18176 # Endere�o do tiro
    	sw $25,200($24) # Endere�o do tiro
    	
    	sw $0,300($24) #Contador de frames
    	
    	
    	lui $25,0x1001
    	addi $25,$25,252 # Endere�o do combustivel
    	sw $25,400($24) 
    	lw $25,400($24)		
    	add $7,$0,$25
    	...
    	lui $25,0x1001 # Endere�o do floco de neve
    	addi $25,$25,184
    	sw $25,700($24)
    	lw $25,700($24)
    	add $7,$0,$25
    	...
    	lui $25,0x1001 # Endere�o do Navio
    	addi $25,$25,320
    	sw $25,800($24)
    	lw $25,800($24)
    	add $7,$0,$25
    	...
    	lui $23,0x1001 # Endere�o do Medidor
    	addi $23,$23,57708
    	sw $23,1300($24)
...
ChecarColisao:
	...
	lw $13,800($24) #Endere�o base do navio
	beq $13,$0,TestaNavio2 #Se n�o houver navio,pula
	
	sub $15,$10,$13 #Diferen�a total de endere�os
	abs $15,$15 # Valor absoluto para n�o importar quem vem antes
	srl $18,$15,9 #Divide por 512 para pegar a diferen�a de linhas
	addi $16,$0,4 #Altura do Navio em linhas
	bgt $18,$16,TestaNavio2  #Se estiver muito longe verticalmente,pula
	
	andi $11,$10,0x1FF #X do tiro (endere�o do tiro and 511)
	andi $12,$13,0x1FF #X do navio(endere�o do navio and 511)
	
	sub $17,$11,$12 #Diferen�a X = X-tiro - X-navio
	abs $17,$17
	
	addi $16,$0,30 #Largura da caixa de colisao
	bgt $17,$16, TestaNavio2 # Se a distancia X for maior que a largura, n�o colidiu
...
ChecaNavio:
	...
	sub $8,$3,$27 #$8 = (Endere�o Avi�o) - (Endere�o Navio)
	abs $8,$8
	li $9,2300    ## 4500 bytes sao quase 9 linhas de dist�ncia
	bgt $8,$9,Retorno # Se a diferen�a > 4500, o avi�o est� muito abaixo do navio
	
	# Teste de colunas
	andi $10,$3,0x1ff # faz um and com endere�o do aviao e 0x1ff para obter a coluna do aviao
	andi $11,$27,0x1ff # faz um and com endere�o do navio e 0x1ff para obter a coluna do navio
	sub $12,$10,$11 # 12 =  distancia entre as colunas
	
	abs $12,$12 # vira positivo
	li $13,45 # Teste se est�o a menos de 45 bytes
...
DesenhaPontos:
	...
	lui $16,0x1001
	lui $22,0x1001
	addi $8,$8,51032 #Endere�o do primeiro numero
	addi $16,$16,51000 #Endere�o do segundo numero
	addi $22,$22,50972 # Endere�o do terceiro numero
...
LoopMenu:
    ...
    j LoopMenu        # Loop infinito na m�sica
...
TocarEVerificar:
    ...
    lw $t7, 4($t8)
    li $t6, 32                # ASCII Espa�o
    beq $t7, $t6, SairSplash  

FimCheck:
    jr $ra                    # Volta para tocar a pr�xima nota

SairSplash:
    # Restauramos tudo e fechamos a pilha
    lw $ra, 0($sp)    # Recupera o endere�o de retorno real
    lw $27, 4($sp)    # Recupera o $27 original
    lw $15, 8($sp)    # Recupera o $15 original
    addi $sp, $sp, 12 # Devolve o espa�o da pilha
...
#-----------------
    ...
    j LoopMenuPerde        # Loop infinito na m�sica
...
VitoriaSom:
    ...
    # Nota 5: C (D� ainda mais agudo) - O Cl�max da vit�ria
    li $a0, 96          
    jal TocarVitoria

    j VitoriaSom

# --- Sub-rotina de Som de Vit�ria ---
TocarVitoria:
    li $a1, 250        # Dura��o bem curta (mais r�pido = mais alegre)
    li $a2, 81         # Instrumento: Lead 8 (Sawtooth) - som de sintetizador de vit�ria
    li $a3, 70         # Volume alto
    li $v0, 33         # Syscall s�ncrona (espera a nota acabar antes de ir pra pr�xima)
    syscall
